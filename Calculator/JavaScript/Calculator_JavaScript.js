...
const Calculator = 
{
    // display default value of 0 on screen
    Display_Value: '0',
    // hold the first operand for any expressions, initialize to null
    First_Operand: null,
    // check whether or not the second operand has been input
    Wait_Second_Operand: false,
    // hold the operator, initialize to null
    operator: null,
};
...
function Input_Digit(digit)
{
    ...
    {   // overwrite Display_Value if the current value is 0, otherwise, it adds onto it
        Calculator.Display_Value = Display_Value === '0' ? digit : Display_Value + digit;
    }
}

// this secondtion handles decimal points
function Input_Decimal(dot)
{
    // this ensures that accidental clicking of the decimal point does not 
    //      cause bugs in mathematical operation
    if(Calculator.Wait_Second_Operand === true) return;
    if(!Calculator.Display_Value.includes(dot))
    {
        // if Display_Value does not contain decimal point, add a decimal point
...
function Handle_Operator(Next_Operator)
{
    ...
    //      in Calculator.First_Operand if it does not already exist
